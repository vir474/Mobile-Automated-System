...
//console.log( jCore.msg );
//console.log(formName + " validation message " + jCore.msg );
//console.log("validation file " + formName);
	// clear any previous msg on top of form
...
//console.log("element v " + typeof jCore.msg['req'] );	
			validateField(formElements[i],formName);
		}
...
//console.log("error fields after " + errorFields.length);
            return false;
        }	
	return true;
}//endfct
...
/**
 * function to show general error message on top of form 
 * */
 
function showGenMessage() {
//console.log("error fields " + errorFields.length);

	var msgElement = document.getElementById('validatemsg');
	for( i=0; i < errorFields.length; i++ ) {
		var tempMsgEle = document.createElement('div');
		
		//create msg element
		tempMsgEle.classList.add("card");
		var msgDiv = '';
		msgDiv += '<div class="item item-divider" style="background-color:#ef4e3a"> Incomplete Form </div>';
		msgDiv += '<div class="item item-text-warp"> Please Enter ' + errorFields[i] + ' </div>';
		tempMsgEle.innerHTML = msgDiv;
		msgElement.appendChild( tempMsgEle );
...
//console.log("validate jcore " + typeof jCore.msg['req']);	
...
//	}
	...
	var parenttemp = obj.parentNode;
	var tempmsgtitle = parenttemp.firstChild.textContent;

	//if we already have an error on the object we dont addup some more messages
/*	if (obj.hasError) {
		errorFields.push(tempmsgtitle);
		return;
	}
*/
	if (W3CDOM) {
...
//console.log("length of msgs " + message.length);
...
//console.log("write error " + obj.name);
//		obj.parentNode.style.backgroundColor = "#ef4e3a";
//		obj.parentNode.style.border-style = "solid";
//		obj.parentNode.style.border-color = "red";
...
//console.log("remove error " + obj.name);
	obj.parentNode.classList.remove('elementerr');
	obj.hasError = null;
	obj.onchange = null;
	
	/*
	// clear any previous msg
	 document.getElementById('validatemsg').innerHTML='';
	 */
}//endfct
